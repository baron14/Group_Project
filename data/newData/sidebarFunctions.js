...
function changedJob(chosen) {								
	...
	$(function () 
	{
		var query="SELECT O.name FROM job_objective O, job_db J, job_objective JO WHERE J.id="+chosen+" AND J.id=JO.job_id AND JO.objective=O.id";
		$.ajax({                                      
			type: 'POST',
			url: 'connect.php',              //the script to call to get data          
			data: "query="+query,            //you can insert url argumnents here to pass to connect.php
	...
	$(function () 
	{
		query="SELECT P.name, P.id FROM parameter P, job_db J, job_parameter JP WHERE J.id="+chosen+" AND J.id=JP.job_id AND JP.parameter=P.id";
		$.ajax({                                      
			type: 'POST',
			url: 'connect.php',              //the script to call to get data          
			data: "query="+query,            //you can insert url argumnents here to pass to connect.php
